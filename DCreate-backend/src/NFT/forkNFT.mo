...
shared(msg)  actor class ForkNFT (paramInfo_: Types.NewForkCollectionParamInfo, uploadBaseFee_: Nat, forkFeeCommission_: Nat, marketFeeRatio_:Nat,
    ...
    private stable var paramInfo: NewForkCollectionParamInfo = paramInfo_;
    ...
    public shared(msg) func uploadIPFSItem(newItem: NewIPFSItem) : async MintResponse {
        ...
        if( Time.now() > paramInfo.deadline or (Time.now() < paramInfo.startline and owner != msg.caller) ){
        ...
        if(msg.caller == owner){
            ...
            var price = paramInfo.forkFee;
            if( paramInfo.whiteListType == 1 and _checkIfInvites(msg.caller) ){
                price := switch(paramInfo.whiteListPrice){
                    case (?p){p};
                    case _ {return #err(#ParamError);};
                };
            }else if(paramInfo.whiteListType == 2 and (not _checkIfInvites(msg.caller))){
                return #err(#NotInWhiteList);
            };
            let protocolDynamicFee:Nat = Nat.div(Nat.mul(price, uploadProtocolFeeRatio), 100);
            tos.add(feeTo);
            values.add(uploadProtocolBaseFee + protocolDynamicFee);
            tos.add(owner);
            values.add(price - protocolDynamicFee);
            switch(newItem.parentToken){
                case(?i){
                    switch(newTokens.get(i)){
                        case(?data){
                            if(not data.bFork){return #err(#ParamError);};
                            if(data.creator != owner){
                                tos.add(data.creator);
                                values.add(price/2);
                            };
                        };
                        case _ {return #err(#ParamError);};
    ...
    public shared(msg) func buyNow(buyRequest: BuyRequest): async BuyResponse {
        ...
        var forkRoyaltyFee = Nat.div(Nat.mul(orderInfo.price, paramInfo.forkRoyaltyRatio), 1000);
        if(forkRoyaltyFee > 0){
    ...
    private func _checkIPFSItemParam(newItem: NewIPFSItem, videoLink: Text) : Result.Result<(), MintError> {
        ...
        if(newItem.desc.size() > maxDescSize
    ...
    public shared(msg) func setMaxParam(newMaxDescSize:Nat) : async Bool {
        assert(msg.caller == controller);
    ...
    public query func getSettings() : async ForkCollectionSettings {
        //fixme: 规避不能返回包含var的结构体的问题
        {
            ...
            newItemForkFee = paramInfo.forkFee;
            totalSupply = if(Time.now() >= paramInfo.deadline){
                owners.size()
            }else{
                totalSupply
            };
            startline = paramInfo.startline;
            deadline = paramInfo.deadline;
            mutablePhotoLink = mutablePhotoLink;
            whiteListType = paramInfo.whiteListType;
    ...
    public query func getSuppy() : async Nat {
        var ret = totalSupply;
        if(Time.now() >= paramInfo.deadline){
            ret := owners.size();
        };
        ret
    };
