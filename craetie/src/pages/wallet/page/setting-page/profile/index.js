...
function ProfileSetting(props) {
  ...
  const handlerSave = () => {
    ...
    let data = {
      ...
      success: () => {
        ...
        profile.avatorCID && isAuthTokenEffect(isAuth, authToken) && dispatch(requestUserAvatar(profile.avatorCID, authToken));
  ...
  return (
    <div>
      ...
      <div className="small-tip  small-tip-666 margin-5">{`* Each person’s information can only be modified 6 times, you still have ${leftCount !== undefined ? leftCount : 6} chances`}</div>
      ...
      <Button type="violet margin-40" onClick={handlerSave} className="btn-small" disabled={changeState === 0 || leftCount === 0}>
        Save
      </Button>
    </div>
  );
