...
export default function MyItemsList(props) {
  const { loading, myItems, isSelf, collectionConfig } = props;
      ...
      let content =
        pageList && pageList.length > 0 ? (
          pageList.map((item, index) => {
            return <CreateNFTCover key={index} baseInfo={item} onButton1Click={onTransferClick} onButtonClick={onButtonClick} />;
          })
        ) : (
          <Empty
            image={<img alt="" src={EmptyImage} />}
            imageStyle={{
              marginTop: '50px',
              height: 100,
            }}
            description={
              <span style={{ color: '#4338CA' }}>
                {'No created...'}
                {isSelf && <a href={`/#/create/nft/${collectionConfig?.length ? collectionConfig[0].key : ''}`}>{'Go to create'}</a>}
              </span>
            }
          ></Empty>
        );
  ...
  return loading ? (
    ...
    <div className="my-items-wrapper">
        ...
        <div className="header">
          ...
          <div className="flex-10">
            <div className="search-input-content">
              ...
              <img alt="" src={ImgSearch} onClick={onSearch} />
            </div>
          </div>
        </div>
      ...
      {listVisible && <SellModal index={curTokenIndex} listVisible={listVisible} type={type} setListVisible={handlerClose} royaltyFee={royaltyFee} forkFee={forkFee} />}
      <ConfirmModal title={'Transfer'} width={328} onModalClose={handlerTransferClose} onModalConfirm={handlerTransfer} modalVisible={transFerVisible}>
        <Input type="text" placeholder={'To Principal ID'} value={trans2Address} className="ant-input-violet input-radius10" onChange={(e) => handleSetTransfer2Address(e)} />
      </ConfirmModal>
    </div>
  );
}
