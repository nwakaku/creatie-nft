...
import { AloneCreate, CrowdCreate, M1155Create, ThemeCreate, Theme1155Create, isCanvas, ProtocolUrl, ArtCollection } from '@/constants';
          ...
          if (exist && index === 0) {
            ...
            for (let item of types) {
              ...
              if (isArt) {
                ...
                title = collection.title;
                imgUrl = `${ProtocolUrl}/resource/${collection.key}/avatar.png`;
              }
        ...
        pageList && pageList.length > 0 ? (
          pageList.map((item, index) => {
            return isCanvas(item.type) ? (
              ...
              <CommonNFTCover key={item.tokenIndex} baseInfo={item} onButton1Click={onButton1Click} onButtonClick={onButtonClick} />
            );
          })
        ) : (
          <Empty
            ...
            description={
              <span style={{ color: '#4338CA' }}>
                {props.nftType === 'nft' ? 'No owned...' : props.nftType === 'drew' ? 'Have not drew...' : 'No favorite'}{' '}
                {isSelf && <a href={props.nftType === 'drew' ? '#/create' : '#/marketplace'}>{props.nftType === 'nft' ? 'Buy now' : props.nftType === 'drew' ? 'Go to drew' : 'Go to market'}</a>}
              </span>
            }
          ></Empty>
        );
          ...
          <div className="header">
            ...
            <div className="flex-10">
              <div className="search-input-content">
                ...
                <img alt="" src={ImgSearch} onClick={onSearch}></img>
              </div>
            </div>
          </div>
