...
import { CrowdCreate, ZombieNFTCreate, ThemeCreate, M1155Create, Theme1155Create, AloneCreate, ArtCollection, isCanvas } from '@/constants';
import './style.less';
...
function ListingsItems(props) {
  ...
  const soldList = isArtCollection ? ['Current Listings', 'Sold Listings', 'All'] : ['Current Listings', 'Sold Listings'];
  ...
  let defaultSoldType = lastInfo?.soldType || (isArtCollection ? 2 : 0);
  if (!isArtCollection && defaultSoldType === 2) {
    defaultSoldType = 0;
  }
  const [soldType, setSoldType] = useState(defaultSoldType); // 0: current listings, 1: history listings
  ...
  const sortList = ['Price: High to Low', 'Price: Low to High', 'Minting #', 'Recently Listed', 'Recently Sold', 'Rarity Score', 'CE'];
  useScrollToTop();
  ...
  const requestData = (nftType, soldType, currentCount) => {
    ...
    if (soldType === 0) dispatch(getCollectionInfo(isArtCollection ? nftType.split(':')[1] : itemConfig?.pricinpalId, nftType, success));
    else if (soldType === 1)
      requestCanister(
  ...
  const getCurPageContent = () => {
    ...
    if (list) {
      ...
      let content = pageList.length ? (
        ...
        <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={soldType === 0 ? 'There are currently no NFTs for sale' : 'There are currently no NFTs for sold'} style={{ width: '100%' }} />
      );
  ...
  const sortMenu = (
    <Menu onClick={onChangeSortType} className="dc-drop-menu">
      ...
      <Content>
        <div className="market-content-wrapper">
          ...
          <div className="header">
            ...
            <div className="market-tabbar-right">
              <div className="search-input-content">
                <Input className="input-style" placeholder="ID Search…" onChange={onInputChange} onPressEnter={onSearch} />
                <img alt="" src={ImgSearch} className="search-img" onClick={onSearch}></img>
              </div>
              <Dropdown className="radio-group dc-dropdown" overlay={soldListMenu} trigger="click">
                <Button className="radioButton">
                  {soldList[soldType]}
                  <img alt="" src={lauActive} className="menu-arrow" />
                  {/* <DownOutlined /> */}
                </Button>
              </Dropdown>
              {((curType !== CrowdCreate && curType !== ThemeCreate) || soldType === 1) && (
                <Dropdown className="radio-group dc-dropdown" overlay={sortMenu} trigger="click">
                  <Button className="radioButton">
                    {sortList[sortType]}
                    <img alt="" src={lauActive} className="menu-arrow" />
                    {/* <DownOutlined /> */}
          ...
          <div className="market-listing">
            {loading ? <Spin style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%, -50%)' }} /> : <div className="nft-list">{getCurPageContent()}</div>}
          </div>
        </div>
      </Content>
