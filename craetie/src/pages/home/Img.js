...
import { isCanvas } from '@/constants';
...
export default ({ item, style, type, index, className }) => {
  ...
  const goToDetail = () => {
    ...
    } else if (isCanvas(item.type)) {
      history.push(`/detail/${item.type}/${item.index}/${item.cid}`);
    } else {
      history.push(`/third/${item.type}/${item.index}/${item.cid}`);
    }
  };
      ...
      case 'showHeader':
        content = (
          <div className="showHeader" style={{ display: 'flex' }}>
            <img alt="" className="img" src={item.owner_avatar_url || DefaultAvator} />
            <div style={{ width: '60%', marginLeft: 10 }}>
              <div className="tip-bold tip-bold-000" style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {item.name}
                {/* {getIndexPrefix(item.type, item.index)} */}
              </div>
              <div className="tip tip-voilet" style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
        ...
        content = (
          <div className={addClass ? addClass : `img-detail`} style={{ display: 'flex' }}>
            ...
            <div style={{ maxWidth: '140px' }}>
              ...
              <div className="value value-voilet" style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {item.owner_name || item.owner_pid}
              </div>
            </div>
          </div>
  ...
  return (
    <div key={index} className={`img-content ${type}-content`} onClick={goToDetail}>
      <img alt="" className={`main ${className || ''}`} style={{ ...style }} src={item.image_url} />
      {changeRender(type)}
    </div>
  );
};
