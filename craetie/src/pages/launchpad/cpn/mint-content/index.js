...
export default memo(function MintContent(props) {
    ...
    return mintPrice.map(([count, price], index) => {
      return (
        ...
        >
          ...
          <WICPPrice iconSize={26} value={price} valueStyle={index === selectIndex ? 'value-white-30' : 'value-30'}></WICPPrice>
        </div>
      );
    });
  };
  ...
  return (
    <div className="mint-list-content">
      {mintPrice.length > 0 ? (
        <div className="content">
          ...
          {parseInt(discount) < 100 && (
            <div className="total">
              After discount:
              <WICPPrice iconSize={20} value={multiBigNumber(mintPrice[selectIndex][1], parseInt(discount) / 100)} valueStyle={'value-20'}></WICPPrice>
            </div>
          )}
          <div className="button-layout">
            <Button type="black" className="btn-normal button" onClick={showConfirmModal} style={{ minWidth: '180px' }} disabled={mintCount >= supplyCount}>
      ...
      {mintPrice.length > 0 && (
        <ConfirmModal title={'Mint confirms'} width={428} onModalClose={cancelMint} onModalConfirm={handlerMintNow} modalVisible={confirmVisible}>
          <div className="tip tip-000" style={{ display: 'flex', alignItems: 'center', columnGap: '10px', marginBottom: '20px' }}>
            Please confirm to pay
            <WICPPrice iconSize={16} value={multiBigNumber(mintPrice[selectIndex][1], parseInt(discount) / 100)} valueStyle={'tip tip-000'}></WICPPrice>
