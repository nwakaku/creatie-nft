...
import { AloneCreate, CrowdCreate, ThemeCreate, M1155Create, Theme1155Create, ZombieNFTCreate, gangNFTCreate } from '../constants';
...
const getUTCTime = (time, fmt = 'MM/dd hh:mm UTC') => {
  ...
  if (/(y+)/.test(fmt)) {
    fmt = fmt.replace(RegExp.$1, (utcDate.getFullYear() + '').substr(4 - RegExp.$1.length));
  }
  let o = {
...
const isVideo = (config, detailUrl) => {
  if (config && config.nftType === 'video' && detailUrl) return true;
...
const getIPFSShowLink = (url) => {
  if (url?.startsWith('https://gateway.filedrive.io/ipfs/')) {
    const cid = url.slice(34);
    const res = 'https://gateway.filedrive.io/ipfs/' + cid.split('.')[0];
    return res;
  }
  return url;
};
