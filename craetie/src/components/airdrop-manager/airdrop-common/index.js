...
import { getIPFSShowLink } from '@/utils/utils';
...
export default memo(function AirDropCommon(props) {
  ...
  const [nftBottom, setNFTBottom] = useState((window.innerHeight * finalBottom) / 100 + (window.innerWidth * boxHeight * finalScale) / 2 / 100);
      ...
      success: (res) => {
        if (mount) {
          ...
          img.onload = () => {
            ...
            setUrl(getIPFSShowLink(url));
            img.onload = null;
          };
          img.src = getIPFSShowLink(url);
        }
      },
      fail: (error) => {
        if (mount) {
  ...
  return (
    <AirdropClaimWrapper>
      {!url && <img alt="" className="blind-bg " src={BlindBg}></img>}
      <Motion style={{ bottom: spring(boxBottom), scale: spring(boxScale) }} onRest={onBoxRest}>
        {({ bottom, scale }) => {
          return (
            <div className={`blind-box`} style={{ transform: `translate(-50%, 50%) scale(${scale})`, bottom: `${bottom}%` }}>
              <img alt="" className={`${loading ? 'vibration' : ''}`} src={BlindBox}></img>
            </div>
          );
        }}
      </Motion>
      {url && (
        <Motion style={{ bottom: spring(nftBottom), scale: spring(nftScale) }} onRest={onNFTRest}>
          {({ bottom, scale }) => {
            // debugger
            return <img alt="" className="nft" src={url} style={{ transform: `translateX(-50%) scale(${scale})`, bottom: `${bottom}px` }}></img>;
          }}
        </Motion>
      )}
      {!url && (
      ...
      {url && closeButton && (
        <div className="success-tip">
          <img alt="" className="tip" src={SuccessTip} />
          <div>
            {'Successfully claim  '}
            <a
          ...
          <img alt="" className="right" src={RightOutline} />
        </div>
      )}
      {!loading && !url && (
        <img
