  ...
  const renderChildren = (data, index) => {
    ...
    return (
      <Dropdown overlay={menu} arrow={false} overlayClassName={'menu-drop'} key={index}>
        <span className="nav-dropdown-link" style={{ color: flag ? '#000' : '#6d7278' }} onClick={(e) => e.preventDefault()}>
          <span>Launchpad</span>
          <img alt="" src={flag ? lauActive : lauDefault} className="menu-arrow" />
        </span>
      </Dropdown>
    );
  };
    ...
    <div className="menu-nav">
      {MenuConfig.map((item, index) => {
        ...
        return item.key === '/create' ? (
          <a className="nav-link" key={item.key} onClick={(e) => onMenuItemClick(e, item.key)}>
            {myIcon(item.icon)}
            <span style={{ marginLeft: 4 }}>{item.title}</span>
          </a>
        ) : (
