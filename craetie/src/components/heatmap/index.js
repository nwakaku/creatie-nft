...
function CanvasHeatMap(props) {
  ...
  const updateHeatMap = (map) => {
    ...
    if (pixelInfo) {
      ...
      for (let piexl of pixelInfo) {
        let pos = piexl[0];
        if (piexl[1].price) {
          let value = Math.ceil(Math.log10(getValueDivide8(getValueMultiplied8(piexl[1].price) / parseInt(canvasInfo.basePrice || 10000))) / Math.log10(1.3)) + 1;
          data.push({
  ...
  return <div id="hotMap" style={{ width: props.showWidth, height: props.showWidth, visibility: props.heatMapShow ? 'visible' : 'hidden' }}></div>;
}
