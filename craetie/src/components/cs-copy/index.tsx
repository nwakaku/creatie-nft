...
const CSCopy = (props: propsType) => {
  ...
  return !copyable ? (
    <Tooltip placement="top" title={tipStr}>
      <Text className={className} style={{ cursor: 'pointer' }} ellipsis={{ suffix }} copyable={false} onClick={handleCopy}>
        {start}...
      </Text>
    </Tooltip>
  ) : (
    <Text className={className} style={{ cursor: 'pointer' }} ellipsis={{ suffix }} copyable={{ text: value, icon: <Image src={CopyIcon} width={14} preview={false} /> }} onClick={handleCopy}>
      {start}...
    </Text>
  );
};
