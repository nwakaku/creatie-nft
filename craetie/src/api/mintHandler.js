...
import { Storage, getItemImageUrl, getIPFSLink } from '@/utils/utils';
...
export async function cliamAirdrop({ type }) {
  ...
  if (res.ok) {
    return {
      ...
      type,
      imgUrl: getIPFSLink(res.ok.photoLink && res.ok.photoLink[0]) || getItemImageUrl(type, res.ok.canisterId, res.ok.index),
      detailUrl: getIPFSLink(res.ok.videoLink && res.ok.videoLink[0]) || `https://${res.ok.canisterId}.raw.ic0.app/token/${res.ok.index}`,
    };
  }
  return res;
}
    ...
    if (mintRes.ok) {
      ...
      for (let item of mintRes.ok) {
        result.push({
          ...
          detailUrl: getIPFSLink(item.videoLink && item.videoLink[0]) || `https://${item.canisterId}.raw.ic0.app/token/${item.index}`,
        });
      }
      return result;
    }
