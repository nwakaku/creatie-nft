...
export async function getMyAllStakingZombieByIndex(data) {
  ...
  let res = fetch?.getUserBonusByIndexs ? await fetch.getUserBonusByIndexs(principal) : await fetch.getUserBonus(principal);
  return res;
}
...
export async function getAllCanvasStakingZombie(data) {
  ...
  let canisterId = canisterManager.getStakeIdByType(type);
  let wicpFecth = await canisterManager.getWICPMotoko(false);
  let balance = await wicpFecth.balanceOf(Principal.fromText(canisterId));
  res.prizePool = balance;
  return res;
}
