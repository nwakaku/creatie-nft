...
export class Transaction {
  ...
  constructor(rosettaTransaction, blockIndex) {
    ...
    if (operations.length >= 2 && operations[1].type === 'TRANSACTION') this.account2Address = operations[1].account.address;
...
export default class RosettaApi {
  ...
  constructor() {
    ...
    this.networkIdentifier = this.networksList().then((res) => res.network_identifiers.find((networkIdentifier) => networkIdentifier.blockchain === 'Internet Computer'));
  }
  ...
  async getTransaction(transactionHash) {
    try {
      ...
      return new Transaction(responseTransactions.transactions[0].transaction, responseTransactions.transactions[0].block_identifier.index);
    } catch (error) {
      //console.log(error);
      return new RosettaError(error.message, axios.isAxiosError(error) ? error?.response?.status : undefined);
    }
