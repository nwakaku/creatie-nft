...
import { ProtocolUrl, ZombieNFTCreate } from '@/constants';
...
function StakingNFTItem(props) {
  let mount = true;
  const { type, multi1155Undone } = props;
  const [staked, setStaked] = useState([]);
  const [reward, setReward] = useState(0);
  const [available, setAvailable] = useState(null);
  const { isAuth, collectionsConfig } = userHooks();
  ...
  const onClickStaking = () => {
    ...
    Dialog.createAndShowDialog(
      <StakingContent available={available} type={type} title={'Staking'} stakeType={'toStake'} multi={type === ZombieNFTCreate ? multi1155Undone : null} staked={staked} />,
      null,
    );
  };
  ...
  const onClickCancelStaking = () => {
    if (staked.length > 0) {
      Dialog.createAndShowDialog(<StakingContent type={type} title={'Cancel Staking'} multi={type === ZombieNFTCreate ? multi1155Undone : null} stakeType={'cancel'} staked={staked} />, null);
    } else {
  ...
  return (
    <>
      <div className="stake-item-wrapper">
        <div className="box">
          <Image width={30} preview={false} src={`${ProtocolUrl}/resource/${type}/avatar.png`} />
        ...
        <div className="box flex">
          ...
          <Button className="stake-btn" style={{ borderRadius: staked.length > 0 ? '0px 10px 10px 0px' : '10px' }} onClick={onClickStaking}>
            Stake
          </Button>
        </div>
      </div>
