...
import { Button, Empty, message, Checkbox } from 'antd';
    ...
    if (list) {
      ...
      let content =
        pageList && pageList.length > 0 ? (
          pageList.map((item) => {
            return <ZombieNFTCover key={item[0]} type={type} tokenIndex={parseInt(item[0])} prinId={item[1]} onItemClick={onItemClick} isSelected={isSelected(parseInt(item[0]))} />;
          })
        ) : (
          <Empty
  ...
  const getCancelContent = () => {
    ...
    if (list) {
      ...
      let content = pageList.map((item) => {
        return (
          <ZombieNFTCover key={item.zombie[0]} type={type} tokenIndex={parseInt(item.zombie[0])} reward={item.zombie[1]} onItemClick={onItemClick} isSelected={isSelected(parseInt(item.zombie[0]))} />
        );
      });
      return content;
    }
  ...
  return (
    <ChangeStakeWrapper>
      ...
      <div className="subTitle">
        ...
        <a className="about" target="_blank" href="https://medium.com/@dfinitygangs/dfinitygangs-staking-summary-319bff75a254">
          About Staking
        </a>
        <div className="subs">
          <div className="tip-000 available">{`Available：${allAvailableNFT.length}`}</div>
          <Checkbox onChange={onChangeSelected} className="stake-check" checked={isToStake ? selected.length === allAvailableNFT.length : selected.length === cancelList.length}>
      ...
      <div className="button-layout">
        <Button type="white-gray" onClick={handlerClose} className="dc-cancel-btn">
          Cancel
        </Button>
        <Button type="violet" onClick={handlerConfirm} className="dc-confirm-btn" disabled={selected.length === 0}>
          Confirm
        </Button>
      </div>
      <ConfirmModal
