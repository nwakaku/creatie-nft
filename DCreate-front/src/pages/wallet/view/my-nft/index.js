...
import { AloneCreate, CrowdCreate, ThemeCreate, M1155Create, Theme1155Create, CollectionType } from '@/constants';
...
import { getBlindBoxStatus, saveNFTInfo, getCreateCollectionSettings } from '@/pages/home/store/actions';
...
export default memo(function MyNFT(props) {
  const NFTS_DATA = 'nftData';
  const ROYALTY_DATA = 'royalty';
  ...
  const { user, nfts, royalty } = useSelector((state) => {
    const user = account === 'wallet' ? authToken : params.user;
    return {
      user,
      nfts: state.allcavans.getIn([`${NFTS_DATA}-${user}`]) || {},
      royalty: state.allcavans.getIn([ROYALTY_DATA]) || {},
    };
  }, shallowEqual);
    ...
    requestCanister(
      getAllNFTByType,
      {
        ...
        success: (res) => {
          ...
          newData[type] = res;
          saveNFTInfo(dispatch, { type: `${NFTS_DATA}-${user}`, value: newData });
        },
      },
      false,
    );
    ...
    requestCanister(
      getRoyaltyFeeRatio,
      {
        ...
        success: (res) => {
          ...
          newData[type] = res;
          saveNFTInfo(dispatch, { type: ROYALTY_DATA, value: newData });
        },
      },
      false,
    );
  ...
  useEffect(() => {
    for (let item of allCreateCollection) {
      ...
      if (parseInt(item.key.split(':')[2]) === CollectionType.CollectionPrivite) {
        requestRoyaltyFee(item.key);
      }
    }
  }, [allCreateCollection, authToken]);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (!isSelf) saveNFTInfo(dispatch, { type: `${NFTS_DATA}-${user}`, value: null });
    };
  }, []);
  ...
  return (
    <div className="nft-content-wrapper">
      ...
      />
      {listVisible && <SellModal index={curTokenIndex} listVisible={listVisible} type={type} setListVisible={handlerClose} royaltyFee={sellRoyalty} forkFee={sellFork} />}
      <ConfirmModal title={'Transfer'} width={428} onModalClose={handlerTransferClose} onModalConfirm={handlerTransfer} modalVisible={transFerVisible}>
        <Input type="text" placeholder={'To Principal ID'} value={trans2Address} className="ant-input-black input-radius6" onChange={(e) => handleSetTransfer2Address(e)} />

        {is1155Canvas(type) && (
          <div>
            <Input type="number" style={{ marginTop: '20px' }} placeholder={'Quantity'} value={quantity} className="ant-input-black input-radius6" onChange={(e) => handlerSetQuantity(e)} />
