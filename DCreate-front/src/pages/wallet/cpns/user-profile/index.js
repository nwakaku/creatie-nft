...
import { requestUserProfile, requestUserRewardsPoints, requestICPBalance, requestWICPBalance, requestUserAvatar } from '@/components/auth/store/actions';
...
function UserProfile(props) {
  ...
  const { isAuth, authToken, wicpBalance, icpBalance } = userHooks();
  const { profile, score, avatar } = useSelector((state) => {
    return {
            ...
            <div style={{ width: '380px' }}>
              <div className="flex-10">
                ...
                <Tooltip placement="top" title={'Receive WICP'}>
                  <Paragraph
                    copyable={{
                      icon: <img alt="" src={copy} style={{ width: '14px', height: '14px' }}></img>,
                    }}
                    ellipsis
                ...
                <Popover placement="rightTop" title={'Principal ID'} content={<Avatar shape="square" src={jrQrcode.getQrBase64(authToken)} size={160} />} trigger="click">
                  <img alt="" src={jrCode} style={{ height: '14px' }} />
                </Popover>
              </div>
              ...
              <div className="flex-10">
                ...
                <Tooltip placement="top" title={'Receive ICP'}>
                  <Paragraph
                    ...
                    style={{ width: '265px', marginBottom: '0px' }}
                  >
                    {isSelf ? principalToAccountId(Principal.fromText(authToken)) : principalToAccountId(Principal.fromText(id))}
                  </Paragraph>
                </Tooltip>
                <Popover
                  title={'Account ID'}
                  content={<Avatar shape="square" src={authToken && jrQrcode.getQrBase64(principalToAccountId(Principal.fromText(authToken)))} size={140} />}
                  trigger="click"
                >
                  <img alt="" src={jrCode} style={{ height: '14px' }} />
                </Popover>
            ...
            content={
              ...
              >
                <a href={shareTwiter} target="_blank" rel="noopener noreferrer" style={{ color: '#000000' }} key={0}>
                  <div className="flex-10">
                    <img alt="" src={Twitter} style={{ height: '20px' }} /> Twitter
                  </div>
                </a>
                ...
                <a href={discordUrl} target="_blank" rel="noopener noreferrer" style={{ color: '#000000' }} key={1}>
                  <div className="flex-10">
                    <img alt="" src={Discord} style={{ height: '20px' }} /> Discord
                  </div>
                </a>
              </div>
            }
        ...
        {profile?.textInfo?.link && (
          <a href={`https://twitter.com/${profile?.textInfo?.link}`} target="_blank" rel="noopener noreferrer" key={0}>
            <div className="flex-5 margin-10">
              <img alt="" src={Twitter} style={{ height: '12px' }} />
        ...
        <div className="bottom">
          <div className="icp-wicp-value">
            <div className="price" style={{ justifyContent: 'flex-end', marginRight: 40 }}>
              <img alt="" src={IcpLogo} style={{ width: '42px', height: '20px' }} />
            ...
            <div className="price" style={{ justifyContent: 'flex-start' }}>
              <img alt="" src={WicpLogo} style={{ width: '25px', height: '25px' }} />
              <span className="number">
                {getValueDivide8(wicpBalance)}
                <span>WICP</span>
              </span>
            </div>
          </div>
