  ...
  const columns = [
    {
      ...
      render: (text, record, index) => {
        ...
        return <div style={{ paddingLeft: imgUrl ? '' : '5px' }}>{imgUrl ? <img alt="" src={imgUrl} style={{ width: '23px', height: '20px' }} /> : index + 1}</div>;
      },
    },
    {
      key: 'name',
      ...
      render: (text, record, index) => {
        return (
          <div className="list-name">
            <img alt="" className="top-img" src={record.logo || ''} />
    ...
    },
    {
      ...
      render: (text, record) => (
        <span>
          <span>
            <img alt="" style={{ width: 20, marginRight: '4px', marginTop: '-4px' }} src={logoSVG} />
          </span>
          {record[`${selectValue}_volume`] ? Number(record[`${selectValue}_volume`] / 100000000).toFixed(2) : record[`${selectValue}_volume`]}
        </span>
      ),
    },
    {
      ...
      render: (text, record) => (
        <span>
          ...
          {record[`current_floor_price`] ? Number(record[`current_floor_price`] / 100000000).toFixed(2) : record[`current_floor_price`]}
      ...
      render: (text, record) => {
        ...
        return (
          <span style={{ color: unit ? '#52C41A' : '#E02020' }}>
            {unit}
            {record[`${selectValue}_gain`] ? Math.round(record[`${selectValue}_gain`] * 100 * 100) / 100 + '%' : record[`${selectValue}_gain`]}
          </span>
        );
      },
    },
