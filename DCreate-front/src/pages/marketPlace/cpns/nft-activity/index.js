...
export default memo(function NFTActivity(props) {
  ...
  const { collectionsConfig, allCreateCollection } = userHooks();
  const { collectionTitle } = useSelector((state) => {
    let collectionTitle;
    if (isArtCollection) {
    ...
    for (let i = 0; i < datas.length; i++) {
      ...
      for (let item of nftData) {
        ...
        transaction.push({
          ...
          imgUrl: getIPFSLink(item.photoLink && item.photoLink[0]) || getItemImageUrl(allTypes[i], item.cid, item.tokenIndex),
          detailUrl: getIPFSLink(item.videoLink && item.videoLink[0]) || `https://${item.cid}.raw.ic0.app/token/${item.tokenIndex}`,
        });
      }
    }
  ...
  useEffect(() => {
    if (collectionTitle) getAllNFTTransactionByType(type);
  }, [collectionTitle]);
