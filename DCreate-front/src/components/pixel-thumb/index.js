...
function PixelThumb(props) {
  ...
  let backgroundColor = props.canvasInfo.backGround !== undefined ? ColorUtils.getColorString(props.canvasInfo.backGround) : '#ffffff';
  ...
  return (
    <div className="pixel-thumb-wrapper" style={{ transform: `translate(-50%, -50%) scale(${props.scale || 1})`, width: `${canvasWidth}px`, height: `${canvasWidth}px` }}>
      <Skeleton.Image />
      {existImage ? (
        <img alt="" src={`https://${prinId}.raw.ic0.app`} style={{ width: '100%', imageRendering: 'pixelated' }} />
      ) : (
        <canvas
