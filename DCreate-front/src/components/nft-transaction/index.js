...
import { formatMinuteSecond, getIPFSShowLink } from '@/utils/utils';
...
export default memo(function NFTTransaction(props) {
  ...
  const onRowClick = (item) => {
    history.push(`/third/${item.type}/${item.index}/${item.prinId}`);
  };
  ...
  const columns = [
    {
      ...
      render: (type, item) => {
        ...
        return (
          ...
          >
            {res.icon && <img alt="" src={res.icon} style={{ height: '18px' }}></img>}
            <div>{res.name}</div>
          </div>
        );
      },
    },
    {
      ...
      render: (title, item) => {
        return (
          ...
          >
            <div style={{ width: '38px', height: '48px', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
              <img style={{ objectFit: 'contain', width: '100%', height: '100%' }} src={getIPFSShowLink(item.imgUrl)}></img>
            </div>
