...
const PictureCarousel = (props) => {
  const { pictures, imgUrl, isScroll } = props;
  const speed = 30;
  ...
  }, [isScroll, isCarousel]); // eslint-disable-line
  ...
  return (
    <PictureListWrapper ref={warper}>
      {!isCarousel && (
        <PictureListItem>
          <img alt="" src={showPicture[showPicture.length - 1]}></img>
        </PictureListItem>
      )}
      {isCarousel && (
