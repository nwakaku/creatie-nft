...
const AirDropManager = () => {
  ...
  const { isAuth, authToken, collectionsConfig } = userHooks();
  const dropList = [];
  ...
  const showDropLayout = (count, type) => {
    ...
    setDropType(type);
    Dialog.createAndShowDialog(type === ZombieNFTCreate ? <Airdrop type={type} /> : type === 'gang' ? <AirdropTiger type={type} /> : <AirdropCommon type={type} />, 0);
  };
