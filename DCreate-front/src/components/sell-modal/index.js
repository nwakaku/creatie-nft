...
export default memo(function SellModal(props) {
  ...
  return (
    ...
    >
      <Input type="number" placeholder="price" className="ant-input-black input-radius4" value={sellPrice} onChange={(e) => handleSetSellPrice(e)} />

      {is1155Canvas(type) && (
        <Input type="number" style={{ marginTop: '20px' }} placeholder="quantity" className="ant-input-black input-radius4" value={sellAmount} onChange={(e) => handleSetSellAmount(e)} />
      )}
      <div className="value value-666 " style={{ marginTop: '18px' }}>
        {`Commision fee ${getMarketFeeRatio(type)}%  For Platform`}
      </div>
      {royaltyFee > 0 && (
        <div className="value value-666 " style={{ marginTop: '8px' }}>
          {`Royalty Fee ${royaltyFee}%  For Creator`}
        </div>
      )}
      {forkFee > 0 && (
        <div className="value value-666 " style={{ marginTop: '8px' }}>
          {`Royalty Fee ${forkFee}%  For Collection Owner`}
        </div>
      )}
    </ConfirmModal>
  );
