...
export default React.memo(() => {
  ...
  const processInfo = [
    { icon: LedgerTransfer, info: 'Ledger Transfer ICP' },
    { icon: MintWicp, info: 'Minting WICP' },
    { icon: Success, info: 'Approving WICP' },
    { icon: Depositing, info: 'Depositing WICP' },
  ];
  ...
  return (
    <div className="icp-wrap-content-wrapper">
      <div className="title title-000"> ICP wrapping in progress </div>
        ...
        {processInfo.map((item, index) => {
          return (
            <div key={index} className="wrap-process-item">
              ...
              {index < processInfo.length - 1 && <img alt="" className="right-arrow" src={RightArrow}></img>}
            </div>
          );
        })}
      </div>
