...
import { InputNumber, Button, Form, Input, message, Image, Slider, Modal } from 'antd';
...
const SideWallet = (props) => {
  const { isAuth, authToken, accountId, wicpBalance, icpBalance } = userHooks();
  ...
  const checkAmount = (_, value) => {
    ...
    } else if (new BigNumber(transAmount).multipliedBy(Math.pow(10, 8)) > icpBalance && (transType === TRANS_TYPE_ICP_ICP || transType === TRANS_TYPE_ICP_WICP)) {
      return Promise.reject(new Error('Insufficient ICP'));
    } else if (new BigNumber(transAmount).multipliedBy(Math.pow(10, 8)) > wicpBalance && (transType === TRANS_TYPE_WICP_WICP || transType === TRANS_TYPE_WICP_ICP)) {
    ...
    let data = {
      ...
      success: async (res) => {
        ...
        if (res) {
          ...
          message.success(transType === TRANS_TYPE_WICP_ICP ? `The ICP will be deposited to your account in a few minutes` : 'successful', transType === TRANS_TYPE_WICP_ICP ? 5 : 3);
  ...
  const handleWrapInput = (value) => {
    ...
    let res = 0;
    if (wrap === 1) {
      if (icpBalance > 0 && value) res = parseFloat(new BigNumber(value).multipliedBy(Math.pow(10, 8)).dividedBy(icpBalance).multipliedBy(100));
      setSliderWrap(res);
    } else if (wrap === -1) {
      if (wicpBalance > 0 && value) res = parseFloat(new BigNumber(value).multipliedBy(Math.pow(10, 8)).dividedBy(wicpBalance).multipliedBy(100));
      setSliderWrap(res);
    }
  ...
  const onChangeWrapValue = (val) => {
    ...
    if (wrap === 1) {
      ...
      res = parseFloat(new BigNumber(wicpBalance).multipliedBy(val).dividedBy(100).dividedBy(Math.pow(10, 8)).toFixed(8));
      setWrapInputValue(parseFloat(res));
    }
    if (res !== null) {
    ...
    <div className="balance-item">
      ...
      <div className="sub-item2">
        <Form.Item name="icp-wrap" rules={[{ validator: checkWrapAmount }]}>
          <InputNumber min={0} keyboard={false} stringMode={true} step={1} placeholder={'0.00'} value={wrapInputValue} onChange={(e) => handleWrapInput(e)} />
        </Form.Item>
      ...
      {wrap === 1 && (
        <div className="slider-layout">
          <Form.Item name="slider">
            <Slider disabled={icpBalance == 0} tipFormatter={(value) => `${value}%`} marks={marks} step={0.01} value={sliderWrap} initialValues={0} onChange={(val) => onChangeWrapValue(val)} />
          </Form.Item>
        </div>
      )}
    </div>
  ...
  const wicpLayout = (
    <div className="balance-item">
      ...
      {wrap === -1 && (
        <div className="slider-layout">
          <Form.Item name="slider">
            <Slider disabled={wicpBalance == 0} tipFormatter={(value) => `${value}%`} marks={marks} step={0.01} value={sliderWrap} initialValues={0} onChange={(val) => onChangeWrapValue(val)} />
          </Form.Item>
        </div>
      )}
    </div>
    ...
    <div className="side-wallet-wrapper">
      ...
      <Form ref={formRef} onFinish={handlerWrapOrUnwrap}>
        ...
        <div className="margin-top-20">
          {isAuth ? (
            <Form.Item>
              <Button type="violet" htmlType="submit" className="wrapp_button btn-normal" loading={transferLoading && transType === -1}>
                {wrap === 1 ? 'Wrap' : 'Unwrap'}
              </Button>
            </Form.Item>
          ) : (
      ...
      <Modal
        ...
        footer={null}
        closable={false}
        title={transType === TRANS_TYPE_ICP_ICP || transType === TRANS_TYPE_WICP_WICP ? 'Transfer' : transType === TRANS_TYPE_ICP_WICP ? 'Wrapped' : 'De-Wrapped'}
        ...
        <Form size="large" autoComplete="off" preserve={false} onFinish={handlerTransfer} ref={modelFormRef}>
          ...
          {transType === TRANS_TYPE_WICP_WICP && (
            <div style={{ marginTop: '15px', color: '#f00', fontSize: '14px', lineHeight: '18px' }}>
              <WarningOutlined />
              ：Attention: WICP transfer only applies to Principal IDs on CCC & Ceto. Transfer to other wallets may cause lost of your assets due to different standards adopted.
            </div>
          )}
          <Form.Item>
            <div className="flex-40" style={{ margin: '10px auto', width: 'fit-content' }}>
              <Button onClick={handleCancleSendModal} className="dc-cancel-btn">
                Cancel
              </Button>
              <Button htmlType="submit" className="dc-confirm-btn">
                Confirm
              </Button>
            </div>
          </Form.Item>
