...
import { authLogin, authLoginOut, initLoginStates, balanceWICP, balanceICP, requestCanister, handleCollectionConfig, promiseAllFunc } from '@/api/handler';
...
const dealWithResult = (dispatch, res, callback) => {
  if (res.ok) {
    ...
    if (!res.ok.status) {
      ...
      dispatch(requestICPBalance(res.ok.prinId));
      dispatch(requestWICPBalance(res.ok.prinId));
    }
  }
