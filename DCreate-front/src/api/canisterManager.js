...
import { idlFactory as FavoriteDIL } from 'canister/local/candid/favoriteStorage.did.js';
import { idlFactory as ICPubCollectionDIL } from 'canister/local/candid/icPubCollection.did.js';
import { idlFactory as ICForkCollectionDIL } from 'canister/local/candid/icForkCollection.did.js';
import { idlFactory as ICPriCollectionDIL } from 'canister/local/candid/icPriCollection.did.js';
...
import { DFINITY_TYPE, PLUG_TYPE, STOIC_TYPE, INFINITY_TYPE, AloneCreate, CrowdCreate, M1155Create, ThemeCreate, ZombieNFTCreate, Theme1155Create, ArtCollection, CollectionType } from '@/constants';
...
const NFTFactoryInfos = {
  ...
  favorite: [FAVORITE_STORAGE_ID, FavoriteDIL],
};
...
class CanisterManager {
    ...
    for (let item of config) {
      ...
      if (!NFTFactoryInfos[type]) {
        ...
        try {
          ...
          else if (item.nftType === 'blindbox') did = require(`canister/local/candid/blindboxNFTFactory.did.js`);
      ...
      if (!NFTStorageInfos[type]) {
        ...
        try {
          ...
          if (item.nftType === 'video' || item.nftType === 'ipfs') did = require(`canister/local/candid/ipfsStorage.did.js`);
  ...
  async getNftFactoryByType(type, isAuth) {
    ...
    if (type?.startsWith(ArtCollection)) {
      ...
      const prinId = temp[1];
      const collectionType = parseInt(temp[2]);
      return this.getCollectionCanister(prinId, collectionType, isAuth);
    }
    let key = type;
    if (isAuth) {
      key += '-auth';
    ...
    if (!canister) {
      ...
      if (info && info[1] && info[0]) {
        ...
        throw new Error(`NFTFactorytype: ${type} not exist `);
      }
    }
    return canister;
  }
  ...
  async getFactoryStorageByType(type) {
    if (type?.startsWith(ArtCollection)) {
      ...
      const prinId = temp[1];
      const collectionType = parseInt(temp[2]);
      return this.getCollectionCanister(prinId, collectionType, false);
    }
    let key = type;
    ...
    if (!canister) {
      ...
      if (info) {
        ...
        throw new Error(`storage type: ${type} not exist `);
      }
    }
    return canister;
  }
  ...
  async getIdentityAgent() {
    ...
    let args = { identity };
    if (process.env.DFX_NETWORK !== 'dev' && process.env.NODE_ENV === 'development') args['host'] = 'https://boundary.ic0.app/';
  ...
  async dealWithPlugIdentityInfo(callback) {
    ...
    if (connected) {
      ...
      const plugPId = await this.plug.getPlugPrinId();
      if (plugPId) {
  ...
  async getStorageMotoko() {
    if (!this.fetchStorageMotoko) this.fetchStorageMotoko = await this.createCanister(StorageDIL, STORAGE_EMAIL_CID, false); //订阅邮件不需要身份
    return this.fetchStorageMotoko;
  }

  async getInternetIdentity() {
    if (!this.interntIdentity) this.interntIdentity = await this.createCanister(InternetIdentiy, 'rdmx6-jaaaa-aaaaa-aaadq-cai', false);
  ...
  async getCollectionCanister(prinId, collectionType, needIdentity) {
    let key = prinId;
    if (this.loginType !== DFINITY_TYPE && needIdentity) {
      key += '-auth';
    } else {
    ...
    if (!canister) {
      ...
      switch (collectionType) {
        case CollectionType.CollectionPublic:
          did = ICPubCollectionDIL;
          break;
        case CollectionType.CollectionFork:
          did = ICForkCollectionDIL;
          break;
        case CollectionType.CollectionPrivite:
          did = ICPriCollectionDIL;
          break;
        default:
          break;
      }
      if (did) {
        canister = await this.createCanister(did, prinId, needIdentity);
        this.canisterMap.set(key, canister);
      }
    }
    return canister;
