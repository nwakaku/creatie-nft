...
import { getItemImageUrl, is1155Canvas, getIPFSLink } from '../utils/utils';
...
export async function getAllNFTByType(data) {
  ...
  let res1 = [];
  for (let item of res) {
    if (is1155Canvas(type)) res1.push({ tokenIndex: item[0].index, prinId: item[0].canisterId, type, rights: item[1], canOperation });
    ...
    if (res) {
      ...
      for (const _nft of res) {
        ...
        let baseInfo = {
          ...
          imgUrl: getIPFSLink(base.photoLink && base.photoLink[0]) || getItemImageUrl(type, base.canisterId, base.index),
          detailUrl: getIPFSLink(base.videoLink && (Array.isArray(base.videoLink) ? base.videoLink[0] : base.videoLink)) || `https://${base.canisterId}.raw.ic0.app/token/${base.index}`,
...
export const getMarketFeePrinID = (type) => {
  ...
  if (isArt) return 'rtn3a-mxwpg-iigeq-tefpm-a6z6k-e3ri4-xbfvj-bjp2e-jrncd-aoljh-uae';
...
export async function getNFTDetailInfo(data) {
  ...
  let res = fetch.getTokenById && (await fetch.getTokenById(tokenIndex));
  return res || {};
}
...
export async function getNFTTotalSupply({ type }) {
  let fetch = await canisterManager.getNftFactoryByType(type, false);
  let res = fetch?.getSuppy && (await fetch.getSuppy());
  return res || 0;
}
...
export async function getNTFTransitionByAccount({ type, prinId }) {
  try {
    ...
    let res =
      ...
      (collectionItem?.nftType === 'blindbox' ? await fetch.getSaleRecordByAccount(Principal.fromText(prinId), key) : await fetch.getSaleRecordByAccount(Principal.fromText(prinId)));
    let set = new Set();
    let indexs = [];
    for (let item of res) {
      item.from && item.from.length && set.add(item.from[0].toText());
...
export async function getAllNTFTransition({ type }) {
  try {
    ...
    let collectionsConfig = canisterManager.getColletcionConfig();
    let collectionItem = find(collectionsConfig, { key: type });
    let res = fetch && fetch.getAllSaleRecord && (collectionItem?.nftType === 'blindbox' ? await fetch.getAllSaleRecord(key) : await fetch.getAllSaleRecord());
    let set = new Set();
    let indexs = [];
    for (let item of res) {
      item.from && item.from.length && set.add(item.from[0].toText());
...
export async function getLinkInfoByIndex(data) {
  let { prinId, type, tokenIndex } = data;
  if (prinId !== 'video') {
    return { photoLink: getItemImageUrl(type, prinId, tokenIndex), videoLink: `https://${prinId}.raw.ic0.app/token/${tokenIndex}` };
  }
  let fetch = await canisterManager.getNftFactoryByType(type, false);
  let res;
  if (['shoe', 'ICDmail'].includes(type)) {
    const photoLink = (await fetch?.getNftPhotoLink(tokenIndex))[0];
    const videoLink = (await fetch?.getNftVideoLink(tokenIndex))[0];
    res = { photoLink: getIPFSLink(photoLink), videoLink: getIPFSLink(videoLink) };
    return res;
  }
  let collectionsConfig = canisterManager.getColletcionConfig();
  let collectionItem = find(collectionsConfig, { key: type });
  if (collectionItem?.nftType === 'blindbox') {
    let infos = await fetch?.getLinkInfoByIndexArr([tokenIndex]);
    if (infos.length) {
      res = { photoLink: getIPFSLink(infos[0][0][0]), videoLink: getIPFSLink(infos[0][1][0]) };
      return res;
    }
  }
  let infos = fetch?.getNftLinkInfo && (await fetch?.getNftLinkInfo(tokenIndex));
  if (infos.length) {
    res = { photoLink: getIPFSLink(infos[0].photoLink), videoLink: getIPFSLink(infos[0].videoLink) };
    return res;
  }
}
